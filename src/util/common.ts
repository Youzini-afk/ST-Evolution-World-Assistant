...
import {
  checkMinimumVersion as _checkMinVersion,
  substituteParams,
} from "../runtime/compat/tools";
...
export function correctlyMerge<TObject, TSource>(
  lhs: TObject,
  rhs: TSource,
): TObject & TSource {
  return _.mergeWith(lhs, rhs, (_lhs, rhs) =>
...
export function regexFromString(
  input: string,
  replace_macros?: boolean,
): RegExp | null {
  if (!input) {
    return null;
  }
